# Créditos de Assets / Asset Credits

Neon Runner 2087 usa assets de terceiros, todos com licença que permite uso comercial
mediante atribuição (ou livre, no caso de CC0). Esta lista deve ser mantida atualizada.

## Kenney City Kit (Commercial 2.1)
- **Autor:** Kenney (www.kenney.nl)
- **Pasta:** `assets/kenney_city-kit-commercial_2.1/`
- **Licença:** Creative Commons Zero (CC0) — uso livre para fins comerciais.
  Atribuição sugerida (opcional): "Kenney - www.kenney.nl"
- **Arquivo de licença:** `assets/kenney_city-kit-commercial_2.1/License.txt`

## Kenney Retro Urban Kit
- **Autor:** Kenney (www.kenney.nl)
- **Arquivo:** `assets/kenney_retro-urban-kit.zip`
- **Licença:** CC0 (mesma política Kenney)

## Ultimate Modular Men Pack
- **Autor:** Quaternius (www.quaternius.com)
- **Arquivo:** `assets/Ultimate Modular Men Pack-glb.zip`
- **Licença:** CC0 (Quaternius disponibiliza os packs gratuitamente para uso comercial)

## Assets do Sketchfab / Poly (GLB) — atribuição por autor
Os modelos abaixo vêm do Sketchfab (alguns originários do Google Poly).
Licenças CC (geralmente CC-BY) — manter a atribuição ao autor em builds e créditos.

| Asset | Autor |
|---|---|
| Apartment building | Poly by Google |
| Bar | Poly by Google |
| Bot Drone | Dave404 |
| Building | Kay Lousberg |
| CAR Model (Lamborghini Aventador) | Ignition Labs |
| Blade Runner Memory Orb | Don Carson |
| Cyberpunk Platform | Quaternius |
| Cyberpunk Signs | Quaternius |
| Drone | NateGazzard |
| Generic Robo dude | Erik Buchholtz |
| Richie | joney_lol |
| Rover | Quaternius |
| Skyscraper | Kenney |
| Spaceship | Quaternius |
| troncityscape1 | Fragmastre TV |
| Turret Gun | Quaternius |
| x-wing | Alberto Calvo (CC-BY) |
| Astronaut (modular men pack) | Quaternius |

## Áudio
- **Música de fundo:** `assets/audio/music.wav` — produção própria.
- Efeitos sonoros: gerados proceduralmente no código (collect/hit).

---

## Arquivos de licença inclusos
- `assets/kenney_city-kit-commercial_2.1/License.txt` (CC0)
- `assets/kenney_city-kit-commercial_2.1/View Documentation.url`
- `assets/kenney_city-kit-commercial_2.1/Visit Kenney.url`

> Nota: conferir a licença exata de cada asset Sketchfab na página do autor antes de
> publicar na loja; esta lista garante a rastreabilidade (procedência + autor).